...
void DoParseAuthority(const CHAR* spec,
                      ...
                      Component* hostname,
                      Component* port_num) {
  CR_DCHECK(auth.is_valid()) << "We should always get an authority";
  if (auth.len == 0) {
    username->reset();
    password->reset();
    hostname->reset();
...
void ParsePath(const CHAR* spec,
  ...
  CR_DCHECK(path.len > 0) << "We should never have 0 length paths";
...
void DoParsePathURL(const CHAR* spec, int spec_len,
  ...
  if (path_begin == spec_len)
    return;
  CR_DCHECK_LT(path_begin, spec_len);
