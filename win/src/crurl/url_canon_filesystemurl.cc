...
#include "crurl/url_parse_internal.h"
...
bool DoCanonicalizeFileSystemURL(const CHAR* spec,
  ...
  bool success = true;
  if (CompareSchemeComponent(spec, inner_parsed->scheme, crurl::kFileScheme)) {
...
bool CanonicalizeFileSystemURL(const crbase::char16* spec,
                               int spec_len,
                               const Parsed& parsed,
                               CharsetConverter* charset_converter,
                               CanonOutput* output,
                               Parsed* new_parsed) {
  return DoCanonicalizeFileSystemURL<crbase::char16, crbase::char16>(
      spec, URLComponentSource<crbase::char16>(spec), parsed, charset_converter,
      output, new_parsed);
}
