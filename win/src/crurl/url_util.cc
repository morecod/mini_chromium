...
#include "crurl/url_file.h"
#include "crurl/url_util_internal.h"
...
bool DoCanonicalize(const CHAR* in_spec,
  ...
  if (DoCompareSchemeComponent(spec, scheme, crurl::kFileScheme)) {
    // File URLs are special.
    ParseFileURL(spec, spec_len, &parsed_input);
    success = CanonicalizeFileURL(spec, spec_len, parsed_input,
                                  charset_converter, output, output_parsed);
  } else if (DoCompareSchemeComponent(spec, scheme, crurl::kFileSystemScheme)) {
  ...
  } else if (DoCompareSchemeComponent(spec, scheme, crurl::kMailToScheme)) {
...
bool DoReplaceComponents(const char* spec,
  ...
  if (DoCompareSchemeComponent(spec, parsed.scheme, crurl::kFileScheme)) {
    return ReplaceFileURL(spec, parsed, replacements, charset_converter, output,
                          out_parsed);
  }
  if (DoCompareSchemeComponent(spec, parsed.scheme, crurl::kFileSystemScheme)) {
  ...
  if (DoCompareSchemeComponent(spec, parsed.scheme, crurl::kMailToScheme)) {
...
void AddStandardScheme(const char* new_scheme,
  ...
  // scheme there.
  CR_DCHECK(!standard_schemes_locked) <<
