...
///#include "crnet/udp/udp_net_log_parameters.h"
...
void UDPSocketWin::Core::ReadDelegate::OnObjectSignaled(HANDLE object) {
  CR_DCHECK_EQ(object, core_->read_overlapped_.hEvent);
  if (core_->socket_)
...
int UDPSocketWin::RecvFrom(IOBuffer* buf,
                           ...
                           CompletionOnceCallback callback) {
  CR_DCHECK(CalledOnValidThread());
  CR_DCHECK_NE(INVALID_SOCKET, socket_);
...
int UDPSocketWin::SendTo(IOBuffer* buf,
    ...
    dscp_manager_->PrepareForSend(address);
  }
...
void UDPSocketWin::LogRead(int result,
  ...
  CR_ALLOW_UNUSED_LOCAL(result);
  CR_ALLOW_UNUSED_LOCAL(bytes);
  CR_ALLOW_UNUSED_LOCAL(address);
}
///
...
int UDPSocketWin::InternalRecvFromOverlapped(IOBuffer* buf,
  ...
  DWORD flags = 0;
  DWORD num;
  CR_CHECK_NE(INVALID_SOCKET, socket_);
...
int UDPSocketWin::RandomBind(const IPAddress& address) {
  CR_DCHECK_EQ(bind_type_, DatagramSocket::RANDOM_BIND);

  for (int i = 0; i < kBindRetries; ++i) {
    int rv = DoBind(IPEndPoint(address, static_cast<uint16_t>(
        cr::RandInt(kPortStart, kPortEnd))));
...
void DscpManager::RequestHandle() {
  ...
  cr::WorkerPool::PostTaskAndReplyWithResult(
      CR_FROM_HERE,
