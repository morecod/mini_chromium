...
#ifndef MINI_CHROMIUM_SRC_CRNET_BASE_IP_ADDRESS_NUMBER_H_
#define MINI_CHROMIUM_SRC_CRNET_BASE_IP_ADDRESS_NUMBER_H_
...
CRNET_EXPORT bool IsIPAddressReserved(const IPAddressNumber& address);

// Returns the string representation of an IP address.
// For example: "192.168.0.1" or "::1".
CRNET_EXPORT std::string IPAddressToString(const uint8_t* address,
                                           size_t address_len);

// Returns the string representation of an IP address along with its port.
// For example: "192.168.0.1:99" or "[::1]:80".
CRNET_EXPORT std::string IPAddressToStringWithPort(const uint8_t* address,
                                                   size_t address_len,
                                                   uint16_t port);

// Same as IPAddressToString() but for an IPAddressNumber.
CRNET_EXPORT std::string IPAddressToString(const IPAddressNumber& addr);

// Same as IPAddressToStringWithPort() but for an IPAddressNumber.
CRNET_EXPORT std::string IPAddressToStringWithPort(const IPAddressNumber& addr,
                                                 uint16_t port);

// Returns the address as a sequence of bytes in network-byte-order.
CRNET_EXPORT std::string IPAddressToPackedString(const IPAddressNumber& addr);

// Parses a URL-safe IP literal (see RFC 3986, Sec 3.2.2) to its numeric value.
// Returns true on success, and fills |ip_number| with the numeric value
CRNET_EXPORT bool ParseURLHostnameToNumber(const std::string& hostname,
...
CRNET_EXPORT_PRIVATE IPAddressNumber ConvertIPv4NumberToIPv6Number(
    const IPAddressNumber& ipv4_number);

// Returns true iff |address| is an IPv4-mapped IPv6 address.
CRNET_EXPORT_PRIVATE bool IsIPv4Mapped(const IPAddressNumber& address);

// Converts an IPv4-mapped IPv6 address to IPv4 address. Should only be called
// on IPv4-mapped IPv6 addresses.
CRNET_EXPORT_PRIVATE IPAddressNumber ConvertIPv4MappedToIPv4(
...
// (IPv6) addresses.
CRNET_EXPORT_PRIVATE bool IPNumberMatchesPrefix(
...
#endif  // MINI_CHROMIUM_SRC_CRNET_BASE_IP_ADDRESS_NUMBER_H_