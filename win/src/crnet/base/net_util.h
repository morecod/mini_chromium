...
CRNET_EXPORT bool ParseHostAndPort(
...
CRNET_EXPORT std::string GetHostAndPort(const GURL& url);

// Returns a host[:port] string for the given URL, where the port is omitted
// if it is the default for the URL's scheme.
CRNET_EXPORT std::string GetHostAndOptionalPort(const GURL& url);
...
CRNET_EXPORT bool IsHostnameNonUnique(const std::string& hostname);

// Returns the hostname of the current system. Returns empty string on failure.
CRNET_EXPORT std::string GetHostName();

// Returns either the host from |url|, or, if the host is empty, the full spec.
CRNET_EXPORT std::string GetHostOrSpecFromURL(const GURL& url);

// Canonicalizes |host| and returns it.  Also fills |host_info| with
// IP address information.  |host_info| must not be NULL.
CRNET_EXPORT std::string CanonicalizeHost(const std::string& host,
...
CRNET_EXPORT bool IsCanonicalizedHostCompliant(const std::string& host);

// Strip the portions of |url| that aren't core to the network request.
//   - user name / password
//   - reference section
CRNET_EXPORT GURL SimplifyUrlForRequest(const GURL& url);
...
///CRNET_EXPORT_PRIVATE bool HasGoogleHost(const GURL& url);
