...
#ifndef MINI_CHROMIUM_CRNET_BASE_IP_ADDRESS_H_
#define MINI_CHROMIUM_CRNET_BASE_IP_ADDRESS_H_
...
class CRNET_EXPORT IPAddressBytes {
 public:
  IPAddressBytes();
...
class CRNET_EXPORT IPAddress {
 public:
  ...
  const uint8_t* data() const { return ip_address_.data(); }
  ...
  bool AssignFromIPLiteral(const cr::StringPiece& ip_literal)
...
CRNET_EXPORT std::string IPAddressToStringWithPort(const IPAddress& address,
                                                   uint16_t port);

// Returns the address as a sequence of bytes in network-byte-order.
CRNET_EXPORT std::string IPAddressToPackedString(const IPAddress& address);

// Converts an IPv4 address to an IPv4-mapped IPv6 address.
// For example 192.168.0.1 would be converted to ::ffff:192.168.0.1.
CRNET_EXPORT IPAddress ConvertIPv4ToIPv4MappedIPv6(const IPAddress& address);

// Converts an IPv4-mapped IPv6 address to IPv4 address. Should only be called
// on IPv4-mapped IPv6 addresses.
CRNET_EXPORT IPAddress ConvertIPv4MappedIPv6ToIPv4(const IPAddress& address);
...
// (IPv6) addresses.
CRNET_EXPORT bool IPAddressMatchesPrefix(const IPAddress& ip_address,
...
//    a:b:c::/46
//    ::1/128
CRNET_EXPORT bool ParseCIDRBlock(const std::string& cidr_literal,
...
CRNET_EXPORT bool ParseURLHostnameToAddress(const cr::StringPiece& hostname,
                                            IPAddress* ip_address)
    CR_WARN_UNUSED_RESULT;

// Returns number of matching initial bits between the addresses |a1| and |a2|.
CRNET_EXPORT unsigned CommonPrefixLength(const IPAddress& a1,
                                         const IPAddress& a2);

// Computes the number of leading 1-bits in |mask|.
CRNET_EXPORT unsigned MaskPrefixLength(const IPAddress& mask);
...
#endif  // MINI_CHROMIUM_CRNET_BASE_IP_ADDRESS_H_