...
bool IsPubliclyRoutableIPv4(const IPAddressBytes& ip_address) {
  ...
  CR_DCHECK_EQ(IPAddress::kIPv4AddressSize, ip_address.size());
  struct {
...
bool IsPubliclyRoutableIPv6(const IPAddressBytes& ip_address) {
  CR_DCHECK_EQ(IPAddress::kIPv6AddressSize, ip_address.size());
  struct {
...
void IPAddressBytes::Assign(const uint8_t* data, size_t data_len) {
  CR_CHECK_GE(16u, data_len);
  size_ = static_cast<uint8_t>(data_len);
...
bool IPAddress::AssignFromIPLiteral(const cr::StringPiece& ip_literal) {
...
IPAddress IPAddress::AllZeros(size_t num_zero_bytes) {
  CR_CHECK_LE(num_zero_bytes, 16u);
  IPAddress result;
...
std::string IPAddress::ToString() const {
  std::string str;
  crurl::StdStringCanonOutput output(&str);

  if (IsIPv4()) {
    crurl::AppendIPv4Address(ip_address_.data(), &output);
  } else if (IsIPv6()) {
    crurl::AppendIPv6Address(ip_address_.data(), &output);
  }
...
IPAddress ConvertIPv4ToIPv4MappedIPv6(const IPAddress& address) {
  ...
  CR_DCHECK(bytes.size() == 16);
  return IPAddress(bytes.data(), bytes.size());
}

IPAddress ConvertIPv4MappedIPv6ToIPv4(const IPAddress& address) {
  CR_DCHECK(address.IsIPv4MappedIPv6());

  std::vector<uint8_t> bytes;
  bytes.reserve(4);
  bytes.insert(bytes.end(),
               address.bytes().begin() + cr_arraysize(kIPv4MappedPrefix),
               address.bytes().end());
  
  CR_DCHECK(bytes.size() == 4);
  return IPAddress(bytes.data(), bytes.size());
}
...
bool IPAddressMatchesPrefix(const IPAddress& ip_address,
  ...
  // or IPv6.
  CR_DCHECK(ip_address.IsValid());
  CR_DCHECK(ip_prefix.IsValid());

  CR_DCHECK_LE(prefix_length_in_bits, ip_prefix.size() * 8);
...
bool ParseCIDRBlock(const std::string& cidr_literal,
  ...
  if (!cr::StringToInt(parts[1], 
                       reinterpret_cast<int*>(&number_of_bits)))
    return false;

  // Make sure the prefix length is in a valid range.
  if (number_of_bits > static_cast<uint32_t>(ip_address->size() * 8))
    return false;
...
bool ParseURLHostnameToAddress(const cr::StringPiece& hostname,
                               IPAddress* ip_address) {
  if (hostname.size() >= 2 && hostname.front() == '[' &&
      hostname.back() == ']') {
    // Strip the square brackets that surround IPv6 literals.
    auto ip_literal =
        cr::StringPiece(hostname).substr(1, hostname.size() - 2);
