...
bool SharedMemory::Create(const SharedMemoryCreateOptions& options) {
  ...
  mapped_file_ = ::CreateFileMappingW(INVALID_HANDLE_VALUE, &sa,
...
bool SharedMemory::Open(const std::string& name, bool read_only) {
  ...
  read_only_ = read_only;
  mapped_file_ =
      OpenFileMappingW(access, false, name_.empty() ? nullptr : name_.c_str());
  if (!mapped_file_)
    return false;
...
bool SharedMemory::ShareToProcessCommon(ProcessHandle process,
  ...
  *new_handle = SharedMemoryHandle();
  ...
  if (process == GetCurrentProcess() && close_self) {
    *new_handle = SharedMemoryHandle(mapped_file, GetCurrentProcId());
    return true;
  }
  ...
  *new_handle = SharedMemoryHandle(result, GetProcId(process));
  return true;
}
