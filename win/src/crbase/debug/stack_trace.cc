...
class SymbolContext {
 public:
  static SymbolContext* GetInstance() {
    ...
    if (!context) {
      SymbolContext* new_context = new SymbolContext();
      if (InterlockedCompareExchangePointer(
          reinterpret_cast<PVOID*>(&context), new_context, NULL)) {
        delete new_context;
      }
    }
    return context;
  ...
  // CR_LOG(FATAL) here because this code is called might be triggered by a
  // CR_LOG(FATAL) itself. Also, it should not be calling complex code that is
...
StackTrace::StackTrace() : StackTrace(cr_arraysize(trace_)) {}

StackTrace::StackTrace(size_t count) {
  count_ = CollectStackTrace(trace_, std::min(count, cr_arraysize(trace_)));
}
...
StackTrace::StackTrace(const _CONTEXT* context) {
  InitTrace(context);
}
...
size_t CollectStackTrace(void** trace, size_t count) {
  ...
  return CaptureStackBackTrace(0, static_cast<DWORD>(count), trace, NULL);
}

void StackTrace::InitTrace(const _CONTEXT* context_record) {
