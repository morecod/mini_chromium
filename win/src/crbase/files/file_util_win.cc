...
bool DeleteFileAfterReboot(const FilePath& path) {
  ...
  return MoveFileExW(path.value().c_str(), NULL,
...
bool PathIsWritable(const FilePath& path) {
  ThreadRestrictions::AssertIOAllowed();
  HANDLE dir =
      ::CreateFileW(path.value().c_str(), FILE_ADD_FILE, kFileShareAll,
...
bool CreateTemporaryFileInDir(const FilePath& dir, FilePath* temp_file) {
  ...
  if (!::GetTempFileNameW(dir.value().c_str(), L"", 0, temp_name)) {
  ...
  DWORD long_name_len = ::GetLongPathNameW(temp_name, long_temp_name, MAX_PATH);
...
bool CreateDirectoryAndGetError(const FilePath& full_path,
  ...
  if (!CreateDirectoryAndGetError(parent_path, error)) {
    ...
    if (error) {
      CR_DCHECK_NE(*error, File::FILE_OK);
    }
    return false;
  }
...
bool CreateWinHardLink(const FilePath& to_file, const FilePath& from_file) {
  ThreadRestrictions::AssertIOAllowed();
  return !!::CreateHardLinkW(to_file.value().c_str(),
                             from_file.value().c_str(), nullptr);
}

bool CreateWinSymbolicLink(const FilePath& to_file, const FilePath& from_file) {
  ThreadRestrictions::AssertIOAllowed();
  DWORD flags = DirectoryExists(from_file) ? SYMBOLIC_LINK_FLAG_DIRECTORY : 0;
  return !!::CreateSymbolicLinkW(
      to_file.value().c_str(), from_file.value().c_str(), flags);
}
...
bool GetFileInfo(const FilePath& file_path, File::Info* results) {
  ...
  if (!::GetFileAttributesExW(file_path.value().c_str(),
...
int WriteFile(const FilePath& filename, const char* data, int size) {
  ...
  win::ScopedHandle file(::CreateFileW(filename.value().c_str(),
                                       ...
                                       NULL));
  if (!file.IsValid()) {
    CR_DPLOG(WARNING) << "CreateFile failed for path " << filename;
    return -1;
  }
  ...
  if (!result) {
    // WriteFile failed.
    CR_DPLOG(WARNING) << "writing file " << filename << " failed";
  } else {
    // Didn't write all the bytes.
    CR_DLOG(WARNING) << "wrote" << written << " bytes to " << filename
                     << " expected " << size;
  }
  return -1;
}
...
bool SetCurrentDirectory(const FilePath& directory) {
  ...
  BOOL ret = ::SetCurrentDirectoryW(directory.value().c_str());
  return ret != 0;
}
...
bool CopyFile(const FilePath& from_path, const FilePath& to_path) {
  ...
  if (!::CopyFileW(from_path.value().c_str(), dest, false)) {
    // Copy failed.
    return false;
  }
