...
std::unique_ptr<Value> Value::CreateNullValue() {
  return std::unique_ptr<Value>(new Value(TYPE_NULL));
}
...
std::unique_ptr<Value> Value::CreateDeepCopy() const {
  return std::unique_ptr<Value>(DeepCopy());
}
...
// static
std::unique_ptr<DictionaryValue> DictionaryValue::From(std::unique_ptr<Value> value) {
  DictionaryValue* out;
  if (value && value->GetAsDictionary(&out)) {
    value.release();
    return std::unique_ptr<DictionaryValue>(out);
  }
  return nullptr;
}
...
void DictionaryValue::Set(const std::string& path, Value* in_value) {
  Set(path, std::unique_ptr<Value>(in_value));
}
...
void DictionaryValue::SetWithoutPathExpansion(const std::string& key,
                                              Value* in_value) {
  SetWithoutPathExpansion(key, std::unique_ptr<Value>(in_value));
}
...
std::unique_ptr<DictionaryValue> DictionaryValue::CreateDeepCopy() const {
  return std::unique_ptr<DictionaryValue>(DeepCopy());
}
...
std::unique_ptr<ListValue> ListValue::From(std::unique_ptr<Value> value) {
  ...
  if (value && value->GetAsList(&out)) {
    value.release();
    return std::unique_ptr<ListValue>(out);
  }
  return nullptr;
}
...
std::unique_ptr<ListValue> ListValue::CreateDeepCopy() const {
  return std::unique_ptr<ListValue>(DeepCopy());
}
