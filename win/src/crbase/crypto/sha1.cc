...
void SHA1HashBytes(const unsigned char* data, size_t len,
 ...
 bool SHA1HashFile(const FilePath& file_name, unsigned char* hash) {
   File file(file_name, File::FLAG_OPEN | File::FLAG_READ);
   if (!file.IsValid())
     return false;
   ...
   char data[1024];
   for (;;) {
     int readcbs = file.ReadAtCurrentPos(data, sizeof(data));
     if (readcbs < 0) { return false; }
     else if (readcbs == 0) { sha.Final(); break; }
     else { sha.Update(data, readcbs); }
   }
