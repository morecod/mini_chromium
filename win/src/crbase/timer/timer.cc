...
void Timer::PostNewScheduledTask(TimeDelta delay) {
  CR_DCHECK(scheduled_task_ == NULL);
  is_running_ = true;
  scheduled_task_ = new BaseTimerTaskInternal(this);
  if (delay > TimeDelta::FromMicroseconds(0)) {
    GetTaskRunner()->PostDelayedTask(posted_from_,
        crbase::Bind(&BaseTimerTaskInternal::Run, 
                     crbase::Owned(scheduled_task_)),
        delay);
  ...
  if (!thread_id_) {
    CR_DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
void Timer::RunScheduledTask() {
  ...
  crbase::Closure task = user_task_;
