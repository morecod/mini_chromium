...
TimerBase::TimerBase(const tracked_objects::Location& posted_from, 
                     TimeDelta delay)
...
void TimerBase::SetTaskRunner(
    scoped_refptr<SingleThreadTaskRunner> task_runner) {
  ///DCHECK(origin_sequence_checker_.CalledOnValidSequence());
  ///CR_DCHECK(task_runner->RunsTasksInCurrentSequence());
  CR_DCHECK(!IsRunning());
  task_runner_.swap(task_runner);
}

void TimerBase::StartInternal(const tracked_objects::Location& posted_from, 
                              TimeDelta delay) {
  ///CR_DCHECK(origin_sequence_checker_.CalledOnValidSequence());
...
scoped_refptr<SingleThreadTaskRunner> TimerBase::GetTaskRunner() {
...
void OneShotTimer::Start(const tracked_objects::Location& posted_from,
                         TimeDelta delay,
...
void OneShotTimer::FireNow() {
  ///CR_DCHECK(origin_sequence_checker_.CalledOnValidSequence());
  CR_DCHECK(!task_runner_) << "FireNow() is incompatible with SetTaskRunner()";
  CR_DCHECK(IsRunning());
...
RepeatingTimer::RepeatingTimer(const tracked_objects::Location& posted_from,
                               TimeDelta delay,
...
void RepeatingTimer::Start(const tracked_objects::Location& posted_from,
                           TimeDelta delay,
                           RepeatingClosure user_task) {
  user_task_ = std::move(user_task);
  StartInternal(posted_from, delay);
...
RetainingOneShotTimer::RetainingOneShotTimer(
...
void RetainingOneShotTimer::Start(const tracked_objects::Location& posted_from,
                                  TimeDelta delay,
                                  RepeatingClosure user_task) {
  user_task_ = std::move(user_task);
  StartInternal(posted_from, delay);
