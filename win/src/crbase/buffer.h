...
#ifndef MINI_CHROMIUM_SRC_CRBASE_MEMORY_BUFFER_H_
#define MINI_CHROMIUM_SRC_CRBASE_MEMORY_BUFFER_H_
...
class BufferT {
 ...
 public:
  ...
  template <typename W,
            typename std::enable_if<
                has_data_and_size<const W, const T>::value>::type* = nullptr>
  ...
  size_t AppendData(size_t max_elements, F&& setter) {
    ...
    size_t written_elements = setter(cr::ArrayView<U>(base_ptr, max_elements));

    CR_CHECK_LE(written_elements, max_elements);
 ...
 private:
  ...
  void ZeroTrailingData(size_t count) {
    CR_DCHECK(IsConsistent());
    CR_DCHECK_LE(count, capacity_ - size_);
  ...
  void OnMovedFrom() {
    CR_DCHECK(!data_);  // Our heap block should have been stolen.
#if CR_DCHECK_IS_ON()
...
#endif  // MINI_CHROMIUM_SRC_CRBASE_MEMORY_BUFFER_H_
