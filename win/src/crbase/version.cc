...
Version::Version(const StringPiece& version_str) {
  std::vector<uint32_t> parsed;
...
// static
bool Version::IsValidWildcardString(const StringPiece& wildcard_string) {
...
int Version::CompareToWildcardString(const StringPiece& wildcard_string) const {
  CR_DCHECK(IsValid());
  CR_DCHECK(Version::IsValidWildcardString(wildcard_string));
