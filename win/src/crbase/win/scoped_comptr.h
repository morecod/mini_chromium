...
#ifndef MINI_CHROMIUM_SRC_CRBASE_WIN_SCOPED_COMPTR_H_
#define MINI_CHROMIUM_SRC_CRBASE_WIN_SCOPED_COMPTR_H_
...
template <class Interface, const IID* interface_id>
class ScopedComPtr : public scoped_refptr<Interface> {
 public:
  ...
  // be confused with e.g. std::unique_ptr::release().
  void Release() {
    if (this->ptr_ != NULL) {
      this->ptr_->Release();
      this->ptr_ = NULL;
  ...
  bool IsSameObject(IUnknown* other) {
    ...
    CR_SCOPED_COMPTR(IUnknown) my_identity;
    QueryInterface(my_identity.Receive());

    ///ScopedComPtr<IUnknown> other_identity;
    CR_SCOPED_COMPTR(IUnknown) other_identity;
