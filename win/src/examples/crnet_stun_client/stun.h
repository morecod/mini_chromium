...
#ifndef MINI_CHROMIUM_SRC_EXAMPLES_CRNET_STUN_H_
#define MINI_CHROMIUM_SRC_EXAMPLES_CRNET_STUN_H_
...
class StunMessage {
 public:
  ...
  std::string password() const {
    CR_DCHECK(integrity_ != IntegrityStatus::kNotSet);
    return password_;
  }
  ...
  // current message.
  bool AddMessageIntegrity32(cr::StringPiece password);
  ...
  static bool IsStunMethod(cr::ArrayView<int> methods,
                           const char* data,
                           size_t size);
  ...
  bool Read(cr::ByteBufferReader* buf);

  // Writes this object into a STUN packet. The return value indicates whether
  // this was successful.
  bool Write(cr::ByteBufferWriter* buf) const;
...
class StunAttribute {
 public:
  ...
  virtual bool Read(cr::ByteBufferReader* buf) = 0;

  // Writes the body (not the type or length) to the given buffer.  Return
  // value is true if successful.
  virtual bool Write(cr::ByteBufferWriter* buf) const = 0;
 ...
 protected:
  ...
  void WritePadding(cr::ByteBufferWriter* buf) const;
  void ConsumePadding(cr::ByteBufferReader* buf) const;
...
class StunAddressAttribute : public StunAttribute {
 public:
  ...
  StunAddressAttribute(uint16_t type, const crnet::IPEndPoint& endpoint);
  ...
  const crnet::IPEndPoint& GetEndPoint() const { return endpoint_; }

  void SetEndPoint(const crnet::IPEndPoint& endpoint) {
    endpoint_ = endpoint;
    EnsureAddressLength();
  }

  bool Read(cr::ByteBufferReader* buf) override;
  bool Write(cr::ByteBufferWriter* buf) const override;
...
class StunXorAddressAttribute : public StunAddressAttribute {
 public:
  StunXorAddressAttribute(uint16_t type, const crnet::IPEndPoint& endpoint);
 ...
 private:
  crnet::IPAddress GetXoredIP() const;
  StunMessage* owner_;
};
...
std::unique_ptr<StunAttribute> CopyStunAttribute(
    ...
    cr::ByteBufferWriter* tmp_buffer_ptr = 0);
...
#endif  // MINI_CHROMIUM_SRC_EXAMPLES_CRNET_STUN_H_
